...
$pageData = array(); // Holds miscellaneous information about the page, such as page and application name.
   ...
   $pageData['pageName'] = "Add User"; // A short, descriptive name for the page. Displayed in the Title Bar.
   $pageData['viewFilename'] = '../../respite/main/userAddView.php'; // The filename of the view that should be invoked.
...
function setPageDefaults($formData, $dbs) {
   ...
   $formData['respite_user'] = 'checked="checked"';
...
function validateSave($validCheckFields, $validRadioFields, $validSelectFields, $validSelectMultiFields, $dbs, $errors, $alerts) {
   ...
   if(!isset($errors['username'])) {
      if(strlen($_POST['username']) > $textMaxLength) {
         $errors['username'] = 'Max length exceeded. Max length is ' . $textMaxLength . ' characters.';
      }
   }
   ...
   if(count($results)!=0) {
      $errors['username'] = 'That username already exists! No duplicates allowed.';
   }
...
function processSave($dbs, $validCheckFields) {
   ...
   // Define the query
   $query = "INSERT INTO {$dbs['respite']['name']}.user (username, name_last, name_first, email, telephone) VALUES (:username, :nameLast, :nameFirst, :email, :telephone)";
   ...
   // Create the user in the role table **********************************

   // Define the query
   $query = "INSERT INTO {$dbs['respite']['name']}.role (username) VALUES (:username)";
