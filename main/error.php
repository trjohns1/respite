...
//
// This is a standalone error page. It displays a generic page with error messages that have been
// set by the calling page in the $_SESSON superglobal.
//
// The calling page must have first invoked session_stat() and set the following variables.
// $_SESSION['error']['type'] // A very short message describing the type of the error.
// $_SESSION['error']['description'] // A description of the error
// $_SESSION['error']['dump'] // Any technical data that should appear that will assist in troubleshooting
//
// The calling page mere sets the variables listed above and issues the following command:
...
$pageData = array(); // Holds miscellaneous information about the page, such as page and application name.
   ...
   $pageData['pageName'] = "Error"; // A short, descriptive name for the page. Displayed in the Title Bar.
   $pageData['viewFilename'] = '../../respite/main/errorView.php'; // The filename of the view that should be invoked.
...
if ($_SERVER['REQUEST_METHOD'] == 'GET') {
   ...
   // Get error messages
   $application['error']['type'] = (isset($_SESSION['error']['type']) ? $_SESSION['error']['type'] : ''); // A very short message describing the type of the error.
   $application['error']['description'] = (isset($_SESSION['error']['description']) ? $_SESSION['error']['description'] : ''); // A description of the error
   $application['error']['dump'] = (isset($_SESSION['error']['dump']) ? $_SESSION['error']['dump'] : ''); // Any technical data that should appear that will assist in troubleshooting
   $application['error']['page'] = (isset($_SERVER['HTTP_REFERER']) ? $_SERVER['HTTP_REFERER'] : ''); // The referring page on which the error occured.

   // Pack presentation variables and display the view
   packView();
   displayView($view, $pageData['viewFilename']);

   // Clear error messages from SESSION so they don't show up elsewhere
