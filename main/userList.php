...
$pageData = array(); // Holds miscellaneous information about the page, such as page and application name.
   ...
   $pageData['pageName'] = "User List"; // A short, descriptive name for the page. Displayed in the Title Bar.
   $pageData['viewFilename'] = '../../respite/main/userListView.php'; // The filename of the view that should be invoked.
...
if ($_SERVER['REQUEST_METHOD'] == 'GET') {
   ...
   // Get the list of users and store in $application
   $application['users'] = getUsers(& $application, $dbs);
...
// * getUsers()
// ****************************************************************************************************
// Get a list of users by username. Call be reference and

function getUsers($application, $dbs) {
   ...
   $query = 'SELECT username, name_first, name_last FROM ' . $dbs['respite']['name'] . ".user" . ' ORDER BY name_last ASC';
   ...
   try {
      ...
      // Display error: err($type, $description, $dump)
      err('Database Error','This system has encountered an error attempting to query the database.');
   }
   ...
   // Unpack the results into an array indexed by username
   foreach($results as $key1=>$value1) {
      $username = $results[$key1]['username'];
      $users[$username]['nameLast'] = $results[$key1]['name_last'];
      $users[$username]['nameFirst'] = $results[$key1]['name_first'];
   }
